...
typedef struct
{
  ...
  bool t1run, t2run;
  bool ca2pulse, cb2pulse;
  bool srtrigger;

  unsigned char cpuIrq;
  unsigned char irqbit;
}viaSave;
...
typedef struct
{
    uint8_t bmode; // Stocke l'état combiné de BC1 et BDIR
    uint8_t creg;  // Stocke le dernier registre sélectionné
    uint8_t colonnes;
}ay8912;

#define AYBMF_BC1  (1 << 0)
#define AYBMF_BDIR (1 << 1)
